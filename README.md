...
工作台将工单量、证据覆盖率、升级率、人工改写率、负向反馈率与 P95 延迟放在同一视图，同时展示知识更新、多语言路由与安全门禁的离线验证结果。

![SupportFlow 运营总览与高风险工单](docs/images/01-overview-high-risk.png)

### 高风险工单：自动升级人工，不生成越权回复

当“隐私泄露投诉”等高风险内容出现时，分流 Agent 标记高风险，质检规则要求升级人工。页面保留质检原因、节点轨迹、人工审核和知识运营入口。

![SupportFlow 高风险工单人工升级](docs/images/02-high-risk-human-review.png)

### 普通工单：RAG 证据与 Agent 决策轨迹

退款咨询会检索退款政策与处理时效等知识证据，并在页面展示来源、版本、相似度、回复草稿和每个 Agent 的处理结果。

![SupportFlow RAG 检索证据与 Agent 轨迹](docs/images/03-rag-evidence-and-trace.png)

### 知识运营：发布、版本化与显式重建索引

管理员发布知识文档后，系统生成版本并显式重建索引；新知识进入后续工单的检索链路，而历史工单保留当时的证据快照。

![SupportFlow 知识发布与索引重建](docs/images/04-knowledge-publish-reindex.png)
